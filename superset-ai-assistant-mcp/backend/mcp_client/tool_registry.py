...
def build_agent_runtime_guidance(superset_public_url: str) -> str:
    return (
        ...
        "- Если пользователь явно указал database/source, сначала подтверди database-level evidence через list_databases, "
        "а уже потом переходи к dataset/table candidate selection.\n"
        "- Для SQL используй execute_sql и соблюдай guardrails на DML/DDL.\n"
        f"- Для ссылок на Superset используй базовый URL: {superset_public_url}. "
        "Оформляй ссылки как markdown: [Открыть график](url), [Открыть дашборд](url), [Открыть SQL Lab](url). "
        "Никогда не вставляй голый URL в текст ответа.\n"
        "- Пиши короткими блоками: один смысловой блок на абзац, без длинных рыхлых абзацев.\n"
        "- Если показываешь SQL, используй только fenced code block ```sql.\n"
        "- Если в запросе есть scope (база/таблица), сначала подтверди database/source, затем резолвь dataset через dataset-level инструменты.\n"
        "- Не используй database/tables endpoint как обязательный шаг для dataset-scoped flows, но database inventory использовать можно и нужно.\n"
        "- Если пользователь указал таблицу/датасет, не подменяй datasource.\n"
        "- Для business-first запросов сначала выбери самый правдоподобный dataset, явно назови допущение и попробуй дать полезный первый ответ.\n"
        "- Задавай уточняющие вопросы про таблицу/метрику/период только если без них ответ будет вводить в заблуждение или не удаётся выбрать plausible datasource.\n"
        "- Если данных недостаточно или возникает ошибка инструмента, задай до 3 точных уточняющих вопросов вместо общего отказа.\n"
    )
