...
def render_chat_window():
    st.markdown(
        ...
        <div class="hero">
          ...
          <p>Единый интерфейс для аналитических запросов, визуализаций и управления артефактами в Apache Superset.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if not st.session_state.messages:
        render_empty_state(
            "Диалог пока пуст",
            "Выберите быстрый вопрос или введите запрос в поле чата ниже.",
        )
    else:
...
def render_us1_window():
    ...
    if status == "idle" and not result:
        render_empty_state(
            ...
            "Нажмите «Запустить сканирование», чтобы собрать схемы, профили таблиц и связи.",
        )
...
def render_us13_window():
    ...
    st.caption(
        "Быстрый сценарий: выберите источник, при необходимости подставьте SQL-шаблон, "
    ...
    with action_col2:
        apply_template_btn = st.button("Подставить SQL-шаблон", use_container_width=True)
    with action_col3:
    ...
    st.selectbox(
        ...
        help="Шаблон можно подставить и при необходимости отредактировать SQL вручную.",
    )
    ...
    st.text_area(
        "SQL для предпросмотра",
        key="us13_sql",
        height=170,
        placeholder="SELECT * FROM birth_names",
    )
    ...
    if not isinstance(preview, dict):
        render_empty_state(
            ...
            "Выберите базу, задайте SQL и нажмите «Запустить предпросмотр».",
        )
        return
    ...
    if sql_executed and st.button("Отправить SQL в чат", use_container_width=True):
...
def render_us14_window():
    ...
    if not isinstance(preview, dict):
        render_empty_state(
            ...
            "Сначала выполните предпросмотр в разделе «Предпросмотр результата».",
        )
    ...
    st.caption("Если поля не выбраны, сервис попробует подобрать их автоматически.")
...
def render_us15_window():
    st.title("Сохранение виджета и share link")
    st.caption(
    ...
    if not datasets:
        render_empty_state(
            ...
            "Нажмите «Обновить датасеты», чтобы загрузить доступные источники из Superset.",
        )
        return
    ...
    if not (numeric_cols or temporal_cols or categorical_cols):
        st.warning(
            "Не удалось получить колонки для датасета. Проверьте metadata датасета "
            "или сначала выполните предпросмотр в разделе US13."
        )
    ...
    if not isinstance(result, dict):
        render_empty_state(
            ...
            "Заполните параметры и нажмите «Создать виджет».",
        )
        return
